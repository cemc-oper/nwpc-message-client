...
func (s *PredictConsumer) ConsumeMessages() error {
	...
	for i := 0; i < s.WorkerCount; i++ {
		go consumePredictMessageToElastic(s, messages)
	}
...
func consumePredictMessageToElastic(consumer *PredictConsumer, messages <-chan amqp.Delivery) {
	...
	for {
		...
		case d := <-messages:
			...
			indexName := getIndexForPredictMessage(event)
		...
		if len(received) >= consumer.BulkSize*10 {
			log.WithFields(log.Fields{
				"component": "elastic",
				"event":     "push",
			}).Fatalf("count of received messages is larger then %d times of bulk size: %d", 10, len(received))
		}
	}
}

func getIndexForPredictMessage(event common.EventMessage) string {
	messageTime := event.Time
	indexName := fmt.Sprintf("predict-%s", messageTime.Format("2006"))
	return indexName
}
